...
И так далее…
Как только ставим цифру в точку финиша, останавливаемся.
Это число и является минимальной длиной пути.
"""
import pygame as pg

from config import COLORS, COLS, SIZE_CELL, ROWS, GOAL, FONT, START
...
class Lee:
    ...
    def make_step(self) -> None:
        ...
        pg.draw.rect(self.screen, COLORS['BLUE'], Cell(*START).rect)  # draw start
        sc_text = self.number.render(str(1), 1, COLORS['WHITE'])  # draw number start
            ...
            for j, _ in enumerate(self.matrix_zero[i]):
                if self.matrix_zero[i][j] == self.step:
                    if i > 0 and self.matrix_zero[i - 1][j] == 0 and grid[i - 1][j] == 0:
                        ...
                        pg.draw.rect(self.screen, COLORS['GREEN'], Cell(i - 1, j).rect)

                        sc_text = self.number.render(str(self.step + 1), 1, COLORS['WHITE'])
                    ...
                    if j > 0 and self.matrix_zero[i][j - 1] == 0 and grid[i][j - 1] == 0:
                        ...
                        pg.draw.rect(self.screen, COLORS['GREEN'], Cell(i, j - 1).rect)
    ...
    def num_step(self) -> None:
        if self.matrix_zero[GOAL[0]][GOAL[1]] == 0:
            self.step += 1
            self.make_step()

    def pave_path(self) -> None:
        """
        ...
        pg.draw.rect(self.screen, COLORS['MAGENTA'], Cell(*GOAL), border_radius=SIZE_CELL // 3)
        ...
        while finish > 1:
            if i > 0 and self.matrix_zero[i - 1][j] == finish - 1:
                ...
                pg.draw.rect(self.screen, COLORS['WHITE'], Cell(j, i).rect, border_radius=SIZE_CELL // 3)
                finish -= 1
